...
var Bus = module.exports = function(_dbus, busName) {
	var self = this;
	...
	function methodCallback(uniqueName, sender, objectPath, interfaceName, member, message, args) {
        self._handleMethodCallback(uniqueName, sender, objectPath, interfaceName, member, message, args);
	}

	function signalCallback(uniqueBusName, sender, objectPath, interfaceName, signalName, args) {
        self._handleSignal(uniqueBusName, sender, objectPath, interfaceName, signalName, args);
	}

	switch(busName) {
	case 'system':
		self.connection = _dbus.getBus(0, methodCallback, signalCallback);
		break;

	case 'session':
		self.connection = _dbus.getBus(1, methodCallback, signalCallback);
		break;
	}
};
...
Bus.prototype._handleMethodCallback = function(uniqueName, sender, objectPath, interfaceName, member, message, args) {
    var self = this;

    for (var hash in self.serviceMap) {
        var service = self.serviceMap[hash];
...
Bus.prototype._handleSignal = function(uniqueBusName, sender, objectPath, interfaceName, signalName, args) {
    var self = this;
...
Bus.prototype.disconnect = function(callback) {
	...
	delete self.signalHandlers[self.connection.uniqueName];
...
Bus.prototype.reconnect = function(callback) {
	...
	self.signalHandlers[self.connection.uniqueName] = self;
...
Bus.prototype.addSignalFilter = function(sender, objectPath, interfaceName, callback) {
	...
	//	self._dbus.addSignalFilter(self.connection, 'type=\'signal\'');
		self._dbus.addSignalFilter(self.connection, 'type=\'signal\',sender=\'org.freedesktop.DBus\',interface=\'org.freedesktop.DBus.Local\',path=\'/org/freedesktop/DBus/Local\',member=\'Disconnected\'');
